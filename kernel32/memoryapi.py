...
VirtualFreeEx = k32_dll.VirtualFreeEx
VirtualFreeEx.restype = wintypes.BOOL
VirtualFreeEx.argtypes = [wintypes.HANDLE, LPVOID, SIZE_T,  DWORD]
...
def _allocate_memory_nr(h_process,num_bytes,protect_flags):
    # Figure out the amt padding.
    mod_amt = PAGE_SZ % num_bytes
    pad_amt = PAGE_SZ - mod_amt
...
def allocate_rwx_memory_nr(h_process,amt):
    return _allocate_memory_nr(h_process,amt,PAGE_EXECUTE_READWRITE)
...
def allocate_rw_memory_nr(h_process,amt):
    return _allocate_memory_nr(h_process,amt,PAGE_READWRITE)
...
def free_memory(hProcess,lpAddress,dwSize,dwFreeType=MEM_DECOMMIT):
